...
@section('content')
    <div class="min-h-screen bg-gray-50">
        <!-- Hero Section -->
        <!-- View & Date Toolbar -->
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 mb-6 flex flex-col sm:flex-row justify-between items-end sm:items-center gap-4">
            
            <!-- Date Navigation (Center/Left) -->
            <div class="flex items-center bg-white p-1 rounded-xl shadow-sm border border-gray-100">
                <a href="{{ route('meeting.room-reservations.index', array_merge(request()->all(), ['date' => \Carbon\Carbon::parse($date)->subDay()->format('Y-m-d')])) }}" 
                   class="p-2 hover:bg-gray-50 rounded-lg text-gray-500 hover:text-green-600 transition-colors">
                    <i class="fas fa-chevron-left"></i>
                </a>
                
                <div class="relative mx-1" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center px-3 py-1.5 space-x-2 text-sm font-bold text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                        <i class="far fa-calendar-alt text-green-500"></i>
                        <span>{{ \Carbon\Carbon::parse($date)->format('d M Y') }}</span>
                    </button>
                    <!-- Simple Date Picker Popup (Native Input fallback for now or Alpine logic) -->
                    <div x-show="open" @click.away="open = false" class="absolute top-full left-0 mt-2 p-2 bg-white rounded-xl shadow-xl border border-gray-100 z-50">
                        <form action="{{ route('meeting.room-reservations.index') }}" method="GET">
                            ...
                            <input type="date" name="date" value="{{ $date }}" 
                                   class="border-gray-200 rounded-lg text-sm focus:border-green-500 focus:ring-green-500" 
                                   onchange="this.form.submit()">
                        </form>
                    </div>
                </div>

                <a href="{{ route('meeting.room-reservations.index', array_merge(request()->all(), ['date' => \Carbon\Carbon::parse($date)->addDay()->format('Y-m-d')])) }}" 
                   class="p-2 hover:bg-gray-50 rounded-lg text-gray-500 hover:text-green-600 transition-colors">
                    <i class="fas fa-chevron-right"></i>
                </a>
                
                @if(\Carbon\Carbon::parse($date)->isToday() == false)
                    <div class="border-l border-gray-200 mx-1 h-6"></div>
                    <a href="{{ route('meeting.room-reservations.index', array_merge(request()->except('date'))) }}" 
                       class="text-xs font-medium text-green-600 px-2 hover:underline">
                        Today
                    </a>
                @endif
            </div>
            ...
            <div class="flex bg-white p-1 rounded-xl shadow-sm border border-gray-100">
                <a href="{{ route('meeting.room-reservations.index', array_merge(request()->except('view'), ['view' => 'grid'])) }}" 
                   class="flex items-center px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ $view === 'grid' ? 'bg-green-50 text-green-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50' }}">
                    <i class="fas fa-th-large mr-2"></i> Room
                </a>
                <a href="{{ route('meeting.room-reservations.index', array_merge(request()->except('view'), ['view' => 'timeline'])) }}" 
                   class="flex items-center px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 {{ $view === 'timeline' ? 'bg-green-50 text-green-700 shadow-sm' : 'text-gray-500 hover:bg-gray-50' }}">
                    <i class="fas fa-stream mr-2"></i> Available Room
                </a>
            </div>
        </div>

        <!-- Search & Filter Bar (Keep existing but inject hidden inputs) -->
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-2 mb-6" id="tour-search-filter">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                        ...
                        <div class="md:col-span-4">
                            ...
                            <div class="relative" x-data="{ 
                                    ...
                                    <div x-show="open" class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-xl py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-xs border border-green-500/30" style="display: none;">
                                        <template x-for="(label, value) in options" :key="value">
                                            <div @click="filter = value; open = false;" class="cursor-pointer select-none relative py-2 pl-3 pr-9 transition-colors duration-150" :class="{ 'text-green-900 bg-green-50': filter == value, 'text-gray-900 hover:bg-green-50 hover:text-green-700': filter != value }">
                                                <span class="block truncate font-medium" :class="{ 'font-semibold': filter == value, 'font-normal': filter != value }" x-text="label"></span>
                                                <span x-show="filter == value" class="absolute inset-y-0 right-0 flex items-center pr-4 text-green-600"><i class="fas fa-check text-xs"></i></span>
                                            </div>
                                        </template>
                                    </div>
                                </div>
                        ...
                        <div class="md:col-span-3 flex space-x-2">
                            ...
                            <a href="{{ route('meeting.room-reservations.index', ['view' => $view, 'date' => $date]) }}" class="px-3 py-1.5 bg-gray-100 text-gray-600 rounded-xl hover:bg-gray-200 font-semibold transition-colors duration-200 flex items-center justify-center text-xs">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 pb-16">
            @if($view === 'timeline')
                @include('meetings.reservation.partials.timeline-view')
            @else
                <!-- Grid View -->
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    @forelse ($rooms as $room)
                        <div class="group relative bg-white rounded-2xl shadow-sm hover:shadow-2xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col h-full transform hover:-translate-y-1" @if($loop->first) id="tour-room-card" @endif>
                            <!-- Image Section -->
                    ...
                    @empty
                        <div class="col-span-full flex flex-col items-center justify-center py-20 px-4">
                            ...
                            <a href="{{ route('meeting.room-reservations.index', ['view' => $view, 'date' => $date]) }}" class="mt-6 text-green-600 hover:text-green-700 font-medium">
                                Clear all filters
                            </a>
                        </div>
                    @endforelse
