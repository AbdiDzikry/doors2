...
@section('title', 'Edit Priority Guest')
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-6 sm:p-8">
                ...
                <div class="flex justify-between items-start">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Edit Priority Guest</h1>
                        <p class="mt-1 text-sm text-gray-500">Update the details for {{ $guest->name }}.</p>
                    </div>
                ...
                <form action="{{ route('master.priority-guests.update', $guest->id) }}" method="POST" class="mt-8 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                            ...
                            <div class="sm:col-span-1 relative">
                                ...
                                <input type="number" name="level" id="level" value="{{ old('level', $guest->level) }}" min="1" max="5"
                                    class="pl-10 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                    required>
                            </div>
                        </div>
