                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                ...
                                <span
                                    class="ml-2 px-2 py-0.5 bg-gray-100 text-[10px] text-gray-500 border border-gray-200 rounded-full font-medium italic">
                                    <i class="fas fa-clock mr-1"></i>Auto-sync: daily
                                    @if(isset($syncStatus))
                                        <span
                                            class="ml-2 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold {{ $syncStatus === 'success' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200' }}">
                            ...
                            <div x-data="{ showImportModal: false }">
                                ...
                                <div x-show="showImportModal" x-cloak
                                    ...
                                    <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md"
                                        ...
                                        <form action="{{ route('master.users.import') }}" method="POST"
                                            enctype="multipart/form-data">
                                            @csrf
                                            <div>
                                                <label for="file" class="block text-sm font-medium text-gray-700">Choose
                                ...
                                <div x-show="showContactModal" x-cloak
                                    ...
                                    <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md"
                                        ...
                                        <p class="text-sm text-gray-500 mb-4">
                                            ...
                                            <a href="{{ route('master.users.contact-template') }}"
                                                class="text-indigo-600 hover:text-indigo-800 font-medium underline">
                                                Download existing data template
                                            </a>
                                        </p>
                                        <form action="{{ route('master.users.import-contacts') }}" method="POST"
                                            enctype="multipart/form-data">
                                            @csrf
                                            <div>
                                                <label for="contact_file"
                                                    class="block text-sm font-medium text-gray-700">Choose file to
                                                    import</label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                                        placeholder="Search by name, email, NPK..."
                                        class="block w-full rounded-lg border-gray-200 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
                                </div>
                                <div>
                                    <label for="role"
                                        ...
                                        <div x-show="open" x-transition:enter="transition ease-out duration-100"
                                            ...
                                            <div @click="selected = ''; open = false"
                                                ...
                                                <span class="block truncate font-medium"
                                                    :class="{ 'font-semibold': selected === '', 'font-normal': selected !== '' }">All
                                                    Roles</span>
                                            ...
                                            @foreach ($roles as $role)
                                                <div @click="selected = '{{ $role->name }}'; open = false"
                                                    ...
                                                    <span class="block truncate font-medium"
                                                        :class="{ 'font-semibold': selected === '{{ $role->name }}', 'font-normal': selected !== '{{ $role->name }}' }">{{ $role->name }}</span>
                                ...
                                <div class="flex items-end space-x-2">
                                    <button type="submit"
                                        class="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg shadow-sm transition-colors text-sm">
                                        Apply Filter
                                    </button>
                                    <a href="{{ route('master.users.index') }}"
                                        class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-medium transition-colors text-sm">
                                        Reset
                                    </a>
                                </div>
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer group hover:bg-gray-100 transition-colors duration-200">
                                            <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'name', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc']) }}"
                                                class="flex items-center space-x-1 w-full h-full text-gray-700 font-bold">
                                                <span>Full Name</span>
                                                @if(request('sort_by', 'name') === 'name')
                                                    <i
                                                        class="fas fa-sort-{{ request('sort_direction', 'asc') === 'asc' ? 'up' : 'down' }} text-green-600"></i>
                                                @else
                                                    <i
                                                        class="fas fa-sort text-gray-300 group-hover:text-gray-400 transition-colors"></i>
                                                @endif
                                            </a>
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer group hover:bg-gray-100 transition-colors duration-200">
                                            <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'npk', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc']) }}"
                                                class="flex items-center space-x-1 w-full h-full text-gray-700 font-bold">
                                                <span>NPK</span>
                                                @if(request('sort_by') === 'npk')
                                                    <i
                                                        class="fas fa-sort-{{ request('sort_direction') === 'asc' ? 'up' : 'down' }} text-green-600"></i>
                                                @else
                                                    <i
                                                        class="fas fa-sort text-gray-300 group-hover:text-gray-400 transition-colors"></i>
                                                @endif
                                        ...
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer group hover:bg-gray-100 transition-colors duration-200">
                                            <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'division', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc']) }}"
                                                class="flex items-center space-x-1 w-full h-full text-gray-700 font-bold">
                                                <span>Division</span>
                                        ...
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer group hover:bg-gray-100 transition-colors duration-200">
                                            <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'position', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc']) }}"
                                                class="flex items-center space-x-1 w-full h-full text-gray-700 font-bold">
                                                <span>Position</span>
                                        ...
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer group hover:bg-gray-100 transition-colors duration-200">
                                            <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'phone', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc']) }}"
                                                class="flex items-center space-x-1 w-full h-full text-gray-700 font-bold">
                                                <span>Phone</span>
                                    ...
                                    @forelse ($users as $user)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {{ $user->npk ?? 'N/A' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {{ $user->division ?? 'N/A' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {{ $user->department ?? 'N/A' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {{ $user->position ?? 'N/A' }}</td>
