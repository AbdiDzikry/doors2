...
@section('content')
    <div class="container-fluid px-6 py-4">
        <div class="py-4">
            ...
            <p class="text-sm text-gray-600">Fill in the details to create a new user account.</p>
        </div>
                        ...
                        <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            @foreach ($roles as $role)
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="roles[]" value="{{ $role->name }}" class="form-checkbox h-5 w-5 text-green-600 rounded focus:ring-green-500" {{ in_array($role->name, old('roles', [])) ? 'checked' : '' }}>
