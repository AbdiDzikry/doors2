            ...
            <div class="p-6 sm:p-8">
                ...
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Update the details for {{ $room->name }}.</p>
                    </div>
                ...
                <form action="{{ route('master.rooms.update', $room->id) }}" method="POST" enctype="multipart/form-data" class="mt-8 space-y-8">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <textarea name="description" id="description" rows="3"
                                    ...
                                    placeholder="A brief description of the room.">{{ old('description', $room->description) }}</textarea>
                            </div>
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <textarea name="facilities" id="facilities" rows="3"
                                    class="block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm"
                                    placeholder="e.g. Whiteboard, Projector, Video Conference">{{ old('facilities', $room->facilities) }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <div class="relative" x-data="{ 
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95"
                                        ...
                                        style="display: none;">
                                        @foreach (['available' => 'Available', 'under_maintenance' => 'Under Maintenance'] as $val => $text)
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <div class="mt-1 flex items-center">
                                    @if($room->image_path)
                                        <img src="{{ route('master.rooms.image', ['filename' => basename($room->image_path)]) }}" alt="{{ $room->name }}" class="w-16 h-16 rounded-md object-cover mr-4">
                                    @endif
