                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                @for ($i = 0; $i < $guest->level; $i++)
                                    <i class="fas fa-star text-yellow-400"></i>
                                @endfor
                                @for ($i = $guest->level; $i < 5; $i++)
                                    <i class="far fa-star text-gray-300"></i>
                                @endfor
                                <span class="ml-2 text-sm text-gray-500">({{ $guest->level }})</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-3">
                                <a href="{{ route('master.priority-guests.edit', $guest->id) }}"
                    ...
                    <tr>
                        <td colspan="3">
                            <div class="text-center py-12">
                                ...
                                <h3 class="mt-2 text-sm font-medium text-gray-900">No Priority Guests Found</h3>
                                <p class="mt-1 text-sm text-gray-500">Get started by adding a new priority guest.</p>
                                <div class="mt-6">
                                    <a href="{{ route('master.priority-guests.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
        ...
        <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md m-4" @click.away="show = false">
            <div class="flex items-start">
                ...
                <div class="ml-4 text-left">
                    <h3 class="text-lg font-semibold text-gray-900">Delete Priority Guest</h3>
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">Are you sure you want to delete this guest? This action cannot be undone.</p>
                    </div>
                </div>
            </div>
