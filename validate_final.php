...
$logFile = __DIR__ . '/logpemesan_fixed.txt';
    ...
    if ($existing) {
        ...
        if ($existingUser && stripos($existingUser->name, $organizerName) !== false) {
            ...
            $report['details'][] = [
                ...
                'expected_npk' => $organizer->npk,
                'current_organizer' => $existingUser ? $existingUser->name : 'Unknown',
                'current_npk' => $existingUser ? $existingUser->npk : 'N/A',
                'room' => $roomName,
...
file_put_contents('validation_report_final.json', json_encode($report, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
echo "Report saved to validation_report_final.json\n";
